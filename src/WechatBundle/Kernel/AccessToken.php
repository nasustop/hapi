...
class AccessToken
{
    ...
    public function refresh()
    {
        ...
        if (empty($response['access_token'])) {
            throw new BadRequestException('Failed to get access_token: ' . json_encode($response, JSON_UNESCAPED_UNICODE));
        }

        $ttl = intval($response['expires_in']) > 5 ? intval($response['expires_in']) - 5 : 0;
        if (! empty($ttl)) {
            $this->cache->set($this->getKey(), $response['access_token'], $ttl);
        }
