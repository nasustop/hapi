...
class Response
{
    public function __construct(protected ?ResponseInterface $response, protected ?BadRequestException $error)
    {
        if (empty($this->response) && empty($this->error)) {
            throw new BadRequestException('Response and error cannot be null at the same time');
        }
    }
    ...
    public function getErrors(): ?BadRequestException
    {
        return $this->error;
    }
    ...
    public function toArray()
    {
        ...
        try {
            ...
            throw new BadResponseException('Could not json decode request body.', $e->getCode(), $e);
        }
